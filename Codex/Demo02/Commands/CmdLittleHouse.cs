...
public class CmdLittleHouse : IExternalCommand
{
    ...
    private const double WallHeightMm = 3000.0;
    ...
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
    {
        ...
        if (document is null)
        {
            ...
            ExecutionLog.Write("LittleHouse.Execute", message);
    ...
    public static Result Execute2(Document document)
    {
        ...
        try
        {
            using var transaction = new Transaction(document, "Create little house");
            transaction.Start();

            var level0 = EnsureLevel(document, report, "Little House Level 0", 0.0);
            var level1 = EnsureLevel(document, report, "Little House Level 3000", WallHeightMm * MillimetersToFeet);

            var boundary = CreateBoundary();
            var walls = CreateWalls(document, report, level0, level1, boundary);
            _ = CreateFloor(document, report, level0, walls, boundary);
            var roof = CreateRoof(document, report, level1, boundary);
            ...
            ExecutionLog.Write("LittleHouse.Execute2", $"Report written to '{mdPath}'.");
            return Result.Succeeded;
        }
        catch (Exception ex)
        {
            ExecutionLog.Write("LittleHouse.Execute2", ex.ToString());
            return Result.Failed;
        }
    }
    ...
    private static bool IsLongEdge(Curve curve)
    {
        if (curve is Line line)
        {
            var dx = Math.Abs(line.GetEndPoint(1).X - line.GetEndPoint(0).X);
            var dy = Math.Abs(line.GetEndPoint(1).Y - line.GetEndPoint(0).Y);
            return dx > dy;
        }
    ...
    private static FamilyInstance CreateHostedInstance(Document document, Wall hostWall, FamilySymbol symbol, XYZ location, Level level, string label, bool flipFacing)
    {
        ...
        ExecutionLog.Write("LittleHouse.Execute2", $"Placed {label} '{symbol.FamilyName}:{symbol.Name}' on wall '{hostWall.Id}'.");
        return instance;
    }
    ...
    public List<FootPrintRoof> Roofs { get; } = [];
        ...
        public string WriteMarkdown()
        {
            ...
            var path = Path.Combine(logDirectory, $"{_stamp}-LittleHouse-report.md");
            var sb = new StringBuilder();
            sb.AppendLine("# Little House Report");
            sb.AppendLine();
            ...
            ExecutionLog.Write("LittleHouse.Report", $"Markdown report written to '{path}'.");
            return path;
        }
    }
